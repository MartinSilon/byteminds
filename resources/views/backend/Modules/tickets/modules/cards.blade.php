    ...
    <div class="offcanvas-body">
        <div class="row">
            <div class="col-9">
                <div class="row">
                    <div class="col-6">
                        <h6 class="mb-1">Klient:</h6>
                        <select class="form-control border" name="client_id" id="client_id">
                            @foreach($clients as $item)
                                <option @if($ticket->client_id == $item->id) selected @endif value="{{ $item->id }}">{{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-6">
                        <h6 class="mb-1">Určené pre:</h6>
                        <select class="form-control border" name="employee_id" id="employee_id">
                ...
                <h6 class="mb-1 mt-4">Popis:</h6>
                <textarea
                    name="description"
                    id="description"
                    class="form-control"
                    rows="15">{{$ticket->description}}</textarea>

                <h6 class="mt-4 mb-1">Cesta k problému:</h6>
                <input
                    type="text"
                    name="url"
                    id="url"
                ...
                <button type="submit" class="form-control mt-3 w-25">Upraviť</button>
            </div>
